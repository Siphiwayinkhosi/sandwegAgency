...
const Faq = () => {
  ...
  return (
    <section className="bg-black text-white font-[Raleway] py-20 px-4 sm:px-10">
      <h2 className="text-4xl sm:text-5xl font-bold text-center mb-12 text-white tracking-wide">
        FAQ
      </h2>

      {/* ===== Scrollable Buttons (Mobile) ===== */}
      <div className="flex justify-center mb-12">
        <div className="flex gap-3 overflow-x-auto scrollbar-hide px-2 sm:px-0 sm:flex-wrap sm:justify-center w-full">
          {faqs.map((f) => (
            <motion.button
              key={f.category}
              onClick={() => setActiveCategory(f.category)}
              whileTap={{ scale: 0.95 }}
              className={`flex-shrink-0 px-4 py-2.5 rounded-full border transition text-sm sm:text-lg whitespace-nowrap
      ...
      <div className="max-w-4xl mx-auto space-y-4">
        {currentFaqs?.items.map((item, idx) => {
          ...
          return (
            <div
              key={id}
              className="border border-gray-700 rounded-lg overflow-hidden backdrop-blur-sm bg-white/5"
            >
              <button
                onClick={() => setOpen(isOpen ? null : id)}
                className="w-full flex justify-between items-center px-5 py-4 text-left hover:bg-gray-900 transition text-base sm:text-lg"
              >
                <span>{item.q}</span>
                <ChevronDown
              ...
              <AnimatePresence initial={false}>
                {isOpen && (
                  <motion.div
                    ...
                    className="px-5 pb-5 text-gray-300 text-sm sm:text-base leading-relaxed"
                  >
                    {item.a}
                  </motion.div>
                )}
